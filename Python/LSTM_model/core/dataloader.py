...
class miniDataLoader():
    ...
    def __init__(self, path ,start_from,cols, returns=True):
        filename = path
        dataframe = pd.read_csv(filename)
        ...
        self.data_train = dataframe.get(cols).to_numpy()
        self.len_train = len(self.data_train)
...
dataset = DataLoader(path='/home/s/Dropbox/KU/BSc Stas/Python/Data/Volatility/'+'DJI'+'.csv',
                         split=0.7, cols=['rv5'], label_col='rv5',returns=False, MinMax=False)